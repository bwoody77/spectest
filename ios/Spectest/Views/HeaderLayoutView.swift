...
@Observable
final class HeaderLayoutViewModel {
  var activeRoute: Any = ""
  var sections: Any = [] as [Any]
  var mobileMenuOpen: Any = false
...
struct HeaderLayoutView: View {
  @State private var vm = HeaderLayoutViewModel()
  var activeRoute: Any = ""
  var sections: Any = [] as [Any]
  init(activeRoute: Any = "", sections: Any = [] as [Any]) { self._vm = State(initialValue: HeaderLayoutViewModel()); self.activeRoute = activeRoute; self.sections = sections }
  var body: some View {
    VStack() {
      HStack(alignment: .center) {
        HStack(alignment: .center, spacing: ThemeManager.shared.size("spacing-3")) {
          Text(verbatim: specString("App"))
            .font(.title3.bold())
        }
          ...
          ForEach(Array(specArr(vm.sections).enumerated()), id: \.offset) { _idx, section in
            HStack(alignment: .center, spacing: ThemeManager.shared.size("spacing-1")) {
              ForEach(Array(specArr(specGet(section, "items")).enumerated()), id: \.offset) { _idx, item in
                Button(action: {  }) {
                  Text(specString(specGet(item, "label")))
    ...
    .onAppear { if !specEq(vm.activeRoute, activeRoute) { vm.activeRoute = activeRoute }; if !specEq(vm.sections, sections) { vm.sections = sections } }
    .task(id: specPropsKey([activeRoute, sections])) { if !specEq(vm.activeRoute, activeRoute) { vm.activeRoute = activeRoute }; if !specEq(vm.sections, sections) { vm.sections = sections } }
  }
}
