...
@Observable
final class TabLayoutViewModel {
  var activeRoute: Any = ""
...
struct TabLayoutView: View {
  @State private var vm = TabLayoutViewModel()
  var activeRoute: Any = ""
  var items: Any = [] as [Any]
  init(activeRoute: Any = "", items: Any = [] as [Any]) { self._vm = State(initialValue: TabLayoutViewModel()); self.activeRoute = activeRoute; self.items = items }
  var body: some View {
    VStack() {
      ...
      HStack(alignment: .center) {
        ForEach(Array(specArr(vm.items).enumerated()), id: \.offset) { _idx, item in
          ...
          VStack(spacing: ThemeManager.shared.size("spacing-1")) {
            Image(systemName: specIconName(specString(specGet(item, "icon"))))
              .font(.system(size: specPx(ThemeManager.shared.resolve("icon-md"))))
              .foregroundStyle((specEq(vm.activeRoute, specGet(item, "id")) ? ThemeManager.shared.color("interactive") : ThemeManager.shared.color("text-secondary")))
    ...
    .onAppear { if !specEq(vm.activeRoute, activeRoute) { vm.activeRoute = activeRoute }; if !specEq(vm.items, items) { vm.items = items } }
    .task(id: specPropsKey([activeRoute, items])) { if !specEq(vm.activeRoute, activeRoute) { vm.activeRoute = activeRoute }; if !specEq(vm.items, items) { vm.items = items } }
  }
}
