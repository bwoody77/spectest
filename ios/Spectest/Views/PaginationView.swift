...
final class PaginationViewModel {
  ...
  var pageSize: Any = 10
  var total: Any? = nil
  var totalPages: Any { ((specDouble(total) > specDouble(0)) ? ((specString((specDouble(total) .truncatingRemainder(dividingBy: specDouble(pageSize)))) == specString(0)) ? (specDouble(total) / specDouble(pageSize)) : specAdd((specDouble((specDouble(total) - specDouble((specDouble(total) .truncatingRemainder(dividingBy: specDouble(pageSize)))))) / specDouble(pageSize)), 1)) : 1) }
  ...
  /* event callback */
          ...
          HStack(alignment: .center, ) {
            if ((specDouble(vm.totalPages) > specDouble(2)) && (specDouble((specDouble(vm.currentPage) - specDouble(1))) > specDouble(1))) {
              Text(specString("\(specString((specDouble(vm.currentPage) - specDouble(1))))"))
                .font(.body.bold())
                .foregroundStyle(ThemeManager.shared.color("semantic.text-secondary"))
            }
          }
